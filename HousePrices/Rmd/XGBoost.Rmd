...
minimum child weight, but for child weight 0, 6 is best.
...
A minimum child weight of 5 with a maximum tree depth of 7 performs best.
...
80% column subsampling by tree with 30% subsampling performed best.
...
```{r}
min(fit$evaluation_log$test_rmse)
...
Maximum depth of 6 and minimum child weight of 1 give the best training error.
In the next grid a smaller area of the same parameter space is searched.
...
It is obvious that the models were overfitted to the training set.
The mean RMSE by cross validation is slightly below 0.12.
The standard errors show, that the optimization in the range below
0.01 RMSE is questionable.
None of both models is significantly better than the other.
