...
In addition the relative cardinality is given.
For discrete features this is expected to be low.
...
To get an idea of how continuous features are distributed, 
several distributions are fit.
The Akaike information criterion is used to decide which distribution best
describes the feature.
This is by no means a complete analysis of distribution.
The intention is to see with which distribution a variable could be compatible.

For each continuous variable a goodness-of-fit estimation is done by minimizing 
the classical Kolmogorov-Smirnov distance.
Since the occurence of 0's can be overwhelming in some variables,
only values above 0 are considered.

```{r}
FitDist <- function(X, distr) {
  vapply(
    seq_len(ncol(X)), function(j) {
      x <- X[[j]]
      x <- x[!is.na(x) & x > 0]
      tryCatch(fitdist(x, distr, method = "mge", gof = "KS")$aic, 
               error = function(e) NA) 
    }, numeric(1)
  )
}
...
Considered are normal, lognormal, and exponential distributions.
The results will be stored in the feature attributes table.

```{r, message=FALSE, warning=FALSE, results='hide'}
df <- data.frame(
  norm = FitDist(full[, contVars, with = FALSE], "norm"),
  lnorm = FitDist(full[, contVars, with = FALSE], "lnorm"),
  exp = FitDist(full[, contVars, with = FALSE], "exp")
)
distros <- apply(df, 1, function(r) names(df)[which.min(r[!is.na(r)])])
featAttr$distro <- ""
featAttr[match(contVars, featAttr$name), distro := distros]
```

```{r}
table(featAttr$distro)
```
